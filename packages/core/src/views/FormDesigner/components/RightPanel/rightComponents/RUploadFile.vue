<template>
  ...
  <a-form-item label="文件类型">
    <fer-select v-model:innerValue="getAccept" placeholder="不限制" allowClear :options="acceptOptions" multiple />
  </a-form-item>
  <a-form-item label="文件大小">
    <a-input-number v-model:value="activeData.fileSize" placeholder="请输入" :min="1" :precision="0">
      <template #addonAfter>
        <a-select v-model:innerValue="activeData.sizeUnit" style="width: 66px">
  ...
  <a-form-item label="上传路径">
    <fer-radio v-model:value="activeData.pathType" :options="pathTypeOptions" optionType="button" button-style="solid" class="right-radio" />
  </a-form-item>
  <template v-if="activeData.pathType === 'selfPath'">
    <a-form-item label="排序规则">
      <fer-select v-model:innerValue="activeData.sortRule" :options="sortRuleOptions" multiple />
    </a-form-item>
    <a-form-item label="时间格式" v-if="activeData.sortRule?.includes('2')">
      <fer-select v-model:innerValue="activeData.timeFormat" :options="timeFormatOptions" />
    </a-form-item>
