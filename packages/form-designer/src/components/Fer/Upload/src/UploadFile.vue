<template>
  <div class="upload-file-container" :class="$attrs.class">
    <template v-if="!detailed">
      <a-button :disabled="disabled" @click="uploadFile" :icon="h(UploadOutlined)">{{ buttonText }}</a-button>
...
<script lang="ts" setup>
  ...
  import { PaperClipOutlined, EyeOutlined, DownloadOutlined, CloseOutlined, UploadOutlined } from '@ant-design/icons-vue';
  // import { getDownloadUrl, getPackDownloadUrl } from '@/api/basic/common';
  import { uploadFileProps } from './props';
  import { useMessage } from '@/hooks/web/useMessage';
  import { useI18n } from '@/hooks/web/useI18n';
  import { toFileSize, downloadByUrl } from '@fer-codeless/utils';

  import FileUploader from './SimpleUploader/FileUploader.vue';

  defineOptions({ name: 'FerUploadFile', inheritAttrs: false });
